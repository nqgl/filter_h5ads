"""
Gene name to Ensembl ID conversion utilities.

Provides functions to convert gene names to Ensembl IDs and filter based on
allowed gene sets, particularly for CRISPR screen data.
"""
...
from filter_h5ads.utils import calculate_filter_stats
...
def convert_gene_labels(
    ...
    if check_conflicts:
        ...
        if overlap:
            raise ValueError(
                f"Conflict detected: Labels appear in both original and converted sets: {overlap}. "
                "This indicates ambiguous gene identifiers that could be interpreted as both "
...
def apply_ensembl_conversion(
    adata: AnnData,
    gene_remapping: dict[str, str],
    mask: list[str] | set[str],
    source_target_column: str = "gene_target",
    target_column_name: str = "target_gene",
    ...
    Args:
        ...
        gene_remapping: Dictionary mapping gene names to Ensembl IDs
        mask: Set of allowed Ensembl IDs to keep
        source_target_column: Column in .obs containing gene targets
        target_column_name: Name for new converted target column
    ...
    # Validate input
    if source_target_column not in adata.obs.columns:
        raise ValueError(
            f"Required column '{source_target_column}' not found in .obs. "
    ...
    # ============================================================================
    logger.info(f"Step 1: Converting {source_target_column} → {target_column_name}")

    # Copy and normalize the gene_target column
    target_genes = adata.obs[source_target_column].copy()
    ...
    adata.obs[target_column_name] = pd.Categorical(converted_targets)

    # Filter cells: keep only those with target_gene in mask or "non-targeting"
    valid_mask = adata.obs[target_column_name].isin(mask_set | {"non-targeting"})
    ...
    adata.var_names = [converted_var_names[i] for i, keep in enumerate(genes_in_mask) if keep]
    n_genes_after_conversion = len(adata.var_names)

    logger.info(
        f"  Converted .var_names: {len(original_var_names):,} → {n_genes_after_conversion:,} genes"
    )
    ...
    if missing_genes:
        logger.info(f"  Adding {len(missing_genes):,} missing genes as zero columns")
        ...
        # Concatenate var DataFrames
        adata.var = pd.concat([adata.var, new_var])
    ...
    if not mask_set.issubset(final_genes_set):
        ...
        raise RuntimeError(
            f"Failed to satisfy mask constraint. Still missing {len(missing_after)} genes: "
            f"{list(missing_after)[:10]}{'...' if len(missing_after) > 10 else ''}"
        )
    ...
    pct_retained = (n_cells_after / n_cells_initial * 100) if n_cells_initial > 0 else 0.0
    ...
    stats = {
        ...
        "source_column": source_target_column,
